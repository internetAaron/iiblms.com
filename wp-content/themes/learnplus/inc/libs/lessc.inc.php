...
class lessc {
	...
	protected function tryImport($importPath, $parentBlock, $out) {
		...
		$url = $this->compileValue($this->lib_ex($str));
	...
	protected function lib_ex($arg) {
		switch ($arg[0]) {
			case "list":
				$items = $arg[2];
				if (isset($items[0])) {
					return $this->lib_ex($items[0]);
				}
	...
	protected function lib__sprintf($args) {
		...
		$template = $this->compileValue($this->lib_ex($string));
		...
		if (preg_match_all('/%[dsa]/', $template, $m)) {
			foreach ($m[0] as $match) {
				...
				$i++;
				$rep = $this->compileValue($this->lib_ex($val));
	...
	protected function lib_unit($arg) {
		if ($arg[0] == "list") {
			...
			return array("number", $this->assertNumber($number),
				$this->compileValue($this->lib_ex($newUnit)));
		} else {
	...
	protected function reduce($value, $forExpression = false) {
		...
		case "interpolate":
			...
			if (empty($value[2])) $res = $this->lib_ex($res);
		...
		case "variable":
			...
			if (is_array($key)) {
				$key = $this->reduce($key);
				$key = $this->vPrefix . $this->compileValue($this->lib_ex($key));
			}
		...
		case "string":
			foreach ($value[2] as &$part) {
				if (is_array($part)) {
					...
					if ($strip) $part = $this->lib_ex($part);
				}
			}
			return $value;
		case "escape":
			list(,$inner) = $value;
			return $this->lib_ex($this->reduce($inner));
		case "function":
