...
class LearnPlus_Recent_Posts_Widget extends WP_Widget {
	/**
	...
	function __construct() {
		$this->defaults = array(
			...
			'length'     => 10,
			'thumb'      => 1,
			'thumb_size' => 'learnplus-widget-thumb',
			'cat'        => '',
			'date'       => 1,
			'comments'   => 0,
			'readmore'   => 1,
			'more_text'  => esc_html__( 'Read More', 'learnplus' ),
		);

		parent::__construct(
			'recent-posts-widget',
			esc_html__( 'LearnPlus - Recent Posts', 'learnplus' ),
			array(
				'classname'   => 'recent-posts-widget',
				'description' => esc_html__( 'Advanced recent posts widget.', 'learnplus' )
			),
			array( 'width' => 590 )
		);
	}
		...
		while ( $query->have_posts() ) : $query->the_post();
			...
			<article class="recent-post <?php echo esc_attr( $class ); ?>">
				...
				<div class="post-text">
					...
					<?php
					if ( $instance['excerpt'] ) {
						echo learnplus_content_limit( get_the_excerpt(), intval( $instance['length'] ), '' );
					}
					if ( $instance['date'] ) {
					...
					if ( $instance['comments'] ) {
						...
						echo '<span class="post-comments">' . sprintf( esc_html__( '%s Comments', 'learnplus' ), intval( $comments_count->total_comments ) ) . '</span>';
					}
	...
	function form( $instance ) {
		...
		<div style="width: 280px; float: left; margin-right: 20px;">
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'limit' ) ); ?>"><?php esc_html_e( 'Number Of Posts', 'learnplus' ); ?></label>
			</p>

			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'cat' ) ); ?>"><?php esc_html_e( 'Select Category: ', 'learnplus' ); ?></label>
				<select class="widefat" multiple="multiple" id="<?php echo esc_attr( $this->get_field_id( 'cat' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'cat' ) ); ?>[]">
					<option value="" <?php selected( empty( $instance['cat'] ) ); ?>><?php esc_html_e( 'All Categories', 'learnplus' ); ?></option>
					<?php
			...
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'thumb' ) ); ?>"><?php esc_html_e( 'Show Thumbnail', 'learnplus' ); ?></label>
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'thumb_size' ) ); ?>"><?php esc_html_e( 'Thumbnail Size', 'learnplus' ); ?></label>
		...
		<div style="width: 280px; float: right;">
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'date' ) ); ?>"><?php esc_html_e( 'Show Date', 'learnplus' ); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'comments' ) ); ?>"><?php esc_html_e( 'Show Comment Number', 'learnplus' ); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'excerpt' ) ); ?>"><?php esc_html_e( 'Show Excerpt', 'learnplus' ); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'length' ) ); ?>"><?php esc_html_e( 'Excerpt Length (words)', 'learnplus' ); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'readmore' ) ); ?>"><?php esc_html_e( 'Show Readmore Text', 'learnplus' ); ?></label>
			</p>

			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'more_text' ) ); ?>"><?php esc_html_e( 'Readmore Text:', 'learnplus' ); ?></label>
