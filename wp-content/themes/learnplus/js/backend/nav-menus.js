...
(function ($) {
	...
	$(function () {
		...
		$( '#menu-to-edit li.menu-item' ).each(function () {
			$(this).find('.item-title').append('<span class="lp-mega-menu">' + learnplus.megamenu + '</span>');
		});
			...
			$.ajax( {
				...
				success: function( response ) {
					$modal.html(
						...
						'<div class="modal-footer"><a href="#" id="modal-save-menu-header" class="button button-primary menu-save">' + learnplus.savemenu + '</a><span class="refresh" style="display: none"> <i class="fa fa-refresh"></i></span></div>' +
						'</div>' +
						'</div>' +
						'</div>');
					...
					$modal.on( 'click', '#modal-save-menu-header', function( e ) {
						...
						if( $( this).parents( '.modal-content').find( '#edit-item-mega-menu-bg_color' ).is(':checked') ) {
							bgColor = 1;
						}
